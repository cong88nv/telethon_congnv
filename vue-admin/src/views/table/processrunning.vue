  ...
  <div class="modal-vue">
    ...
    <div v-if="open" class="modal">
      ...
      <el-form ref="form" :model="addReplaceWords" label-width="120px">
        <el-form-item label="Replace category">
        ...
        <el-form-item label="Word">
          <el-input v-model="addReplaceWords.list_keyword" />
        </el-form-item>
        <el-form-item label="Replace">
          <el-input v-model="addReplaceWords.replace" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="Idx" width="150">
          <template slot-scope="scope">
            {{ scope.row.id }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    addCate() {
      ...
      fetch('http://ssh.lalasoft.vn:8001/replace-words', requestOptions).then(async response => {
        this.$router.go(this.$router.currentRoute)
      })
    },
    getReplaceWorlds() {
      this.listLoading = true
      fetch('http://ssh.lalasoft.vn:8001/replace-words').then(async response => {
    ...
    getCategory(id) {
      fetch('http://ssh.lalasoft.vn:8001/categories?cate_for=filter&type_id=4').then(async response => {
        const datas = await response.json()
